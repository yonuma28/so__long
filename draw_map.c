...
static void	*load_img_checked(void *mlx_ptr, char *path, int *width, int *height)
{
	void	*img;
...
{
	imgs->window = load_img_checked(map->mlx, map->texture.img_window,
			&map->texture.img_width, &map->texture.img_height);
	imgs->wall = load_img_checked(map->mlx, map->texture.img_wall,
			&map->texture.img_width, &map->texture.img_height);
	imgs->pc1 = load_img_checked(map->mlx, map->texture.img_PC1,
			&map->texture.img_width, &map->texture.img_height);
	imgs->pc2 = load_img_checked(map->mlx, map->texture.img_PC2,
			&map->texture.img_width, &map->texture.img_height);
	imgs->exit_img = load_img_checked(map->mlx, map->texture.img_E,
			&map->texture.img_width, &map->texture.img_height);
	imgs->person1 = load_img_checked(map->mlx, map->texture.img_Person1,
			&map->texture.img_width, &map->texture.img_height);
	imgs->person2 = load_img_checked(map->mlx, map->texture.img_Person2,
			&map->texture.img_width, &map->texture.img_height);
	imgs->collectible = load_img_checked(map->mlx, map->texture.img_tea,
			&map->texture.img_width, &map->texture.img_height);
	imgs->empty_space = load_img_checked(map->mlx, map->texture.img_0,
			&map->texture.img_width, &map->texture.img_height);
	imgs->enemy_img = load_img_checked(map->mlx, map->texture.enemy,
			&map->texture.img_width, &map->texture.img_height);
	imgs->object_img = load_img_checked(map->mlx, map->texture.object,
			&map->texture.img_width, &map->texture.img_height);
}
...
static void	draw_wall_variant_tile(t_map *map, t_images *imgs,
		t_coord pos, bool *next_pc1_flag)
{
	void	*img_to_draw;
	...
	mlx_put_image_to_window(map->mlx, map->win, img_to_draw,
		pos.x * map->texture.img_width, pos.y * map->texture.img_height);
}

static void	draw_player_tile_animated(t_map *map, t_images *imgs,
		t_coord pos, bool *anim_state)
{
	void	*img_to_draw;
...
static void	draw_single_tile(t_map *map, t_images *imgs,
		t_coord pos, t_render_flags *flags)
{
	char	tile_char;
	...
	if (tile_char == '1')
		...
		mlx_put_image_to_window(map->mlx, map->win, imgs->exit_img,
			pos.x * map->texture.img_width, pos.y * map->texture.img_height);
	else if (tile_char == 'P')
		draw_player_tile_animated(map, imgs, pos, flags->player_anim_state);
	else if (tile_char == 'C')
		mlx_put_image_to_window(map->mlx, map->win, imgs->collectible,
			pos.x * map->texture.img_width, pos.y * map->texture.img_height);
	else if (tile_char == '0')
		mlx_put_image_to_window(map->mlx, map->win, imgs->empty_space,
			pos.x * map->texture.img_width, pos.y * map->texture.img_height);
	else if (tile_char == 'O')
		mlx_put_image_to_window(map->mlx, map->win, imgs->object_img,
			pos.x * map->texture.img_width, pos.y * map->texture.img_height);
	else if (tile_char == 'N')
		mlx_put_image_to_window(map->mlx, map->win, imgs->enemy_img,
			pos.x * map->texture.img_width, pos.y * map->texture.img_height);
}

static void	render_map_tiles(t_map *map, t_images *imgs, bool *player_anim_state)
{
	t_coord			pos;
...
static void	display_game_hud(t_map *map)
{
	...
	while (dx <= 1)
	{
		...
		while (dy <= 1)
		{
			mlx_string_put(map->mlx, map->win, 10 + dx, 10 + dy,
				0xFF1493, "count: ");
			mlx_string_put(map->mlx, map->win, 100 + dx, 10 + dy,
				0xFF1493, step_buffer);
			dy++;
		}
		dx++;
	}
